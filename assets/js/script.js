...
function injectSitePolishStyles() {
  ...
  style.textContent = `
    :root { --ds-emerald: rgba(15,60,53,1) !important; --ds-green-pigment: rgba(11,47,42,1) !important; }
    .hero-trust-row { display:flex; align-items:center; justify-content:center; flex-wrap:nowrap; margin-block:16px 20px; color:rgba(15,60,53,1); font-family:var(--ff-body,Roboto,sans-serif); font-size:1.25rem; font-weight:700; letter-spacing:.4px; white-space:nowrap; }
    .hero-trust-row span { display:inline-flex; align-items:center; flex:0 0 auto; }
    .hero-trust-row span + span::before { content:'•'; margin-inline:10px; opacity:.65; }
    ...
    @media (max-width:767px) {
      .hero-trust-row { justify-content:flex-start; overflow-x:auto; overflow-y:hidden; -webkit-overflow-scrolling:touch; scrollbar-width:none; font-size:1.05rem; padding-block:2px 4px; margin-block:12px 18px; }
...
function ensureHeroTrustRow() {
  const heroContent = document.querySelector('.hero-content');
  const heroButtons = document.querySelector('.hero-btn-group');
  if (!heroContent || !heroButtons || document.querySelector('.hero-trust-row')) return;
  const row = document.createElement('div');
  row.className = 'hero-trust-row';
  row.setAttribute('aria-label','Herbiveda trust signals');
  row.innerHTML = '<span>BAMS-reviewed</span><span>Batch COAs</span><span>Cited studies</span><span>Traditional use clearly labeled</span>';
  heroContent.insertBefore(row, heroButtons);
}
function addFourthHomepageCard(selector, cardHtml) {
  const list = document.querySelector(selector);
  if (!list) return;
  Array.from(list.children).forEach(function (item) { if (/coming\s*soon/i.test(item.textContent || '')) item.remove(); });
...
function initializeTestimonialsSwiper() {
  ...
  Array.from(list.children).forEach(function (slide) { slide.classList.add('swiper-slide'); wrapper.appendChild(slide); });
  ...
  new window.Swiper('.testimonials-swiper', { slidesPerView:1, spaceBetween:18, speed:650, loop:true, grabCursor:true, autoplay:{delay:5000,disableOnInteraction:false}, pagination:{el:'.testimonials-swiper .swiper-pagination',clickable:true}, navigation:{nextEl:'.testimonials-swiper .swiper-button-next',prevEl:'.testimonials-swiper .swiper-button-prev'}, breakpoints:{768:{slidesPerView:2,spaceBetween:20},1024:{slidesPerView:3,spaceBetween:20}} });
}
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initializeHomepageEnhancements, { once:true });
else initializeHomepageEnhancements();
